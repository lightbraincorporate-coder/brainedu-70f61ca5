      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 flex items-center gap-2 text-sm text-muted-foreground flex-wrap select-none">
          ...
          {selectedTrimester && (
            <>
              ...
              <span 
                className="cursor-pointer hover:text-foreground hover:underline transition-colors"
                onClick={() => setSelectedExpose(null)}
              >
                {selectedTrimester}
              </span>
            </>
          ...
          {selectedExpose && (
            <>
              <span>/</span>
              <span className="text-foreground font-medium">{currentExpose?.title}</span>
            </>
          )}
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {getExposesForTrimester().map((expose, index) => (
                <Card 
                  ...
                  className="cursor-pointer hover:shadow-lg transition-all hover:-translate-y-1 animate-scale-in"
                  style={{ animationDelay: `${index * 100}ms` }}
                  onClick={() => setSelectedExpose(expose.id)}
                >
                  <CardHeader>
                    <CardTitle className="text-base">{expose.title}</CardTitle>
                  </CardHeader>
                </Card>
              ))}
            </div>
        ...
        {selectedExpose && currentExpose && (
          <div className="animate-fade-in space-y-6">
            <div className="space-y-4">
              ...
              {linkedCourse && (
                <p className="text-sm text-muted-foreground">
                  Exposé sur le cours : <span className="font-semibold">{linkedCourse.title}</span>
                </p>
              )}

              {/* Download Button */}
              {currentExpose.driveLink && (
                <Button 
                  size="lg"
                  onClick={() => handleDownload(currentExpose.driveLink)}
                  className="gap-2"
                >
            ...
            {/* Back to Exposes Card */}
            <Card 
              className="cursor-pointer hover:shadow-lg transition-all hover:-translate-y-1"
              onClick={() => setSelectedExpose(null)}
            >
              <CardHeader>
                <CardTitle>← Retour à la liste des exposés</CardTitle>
              </CardHeader>
            </Card>
          </div>
        )}
